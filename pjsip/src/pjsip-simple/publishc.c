...
PJ_DEF(pjsip_from_hdr *) pjsip_publishc_get_from_hdr(pjsip_publishc *pubc)
{
	return pubc->from_hdr;
}
					   ...
					   pjsip_publishc **p_pubc)
{
    ...
    pubc->token = token;
    pubc->cb = cb;
	pubc->refresh_timer_cb = refresh_timer_cb;
...
static void tsx_callback(void *token, pjsip_event *event)
{
	...
	if (tsx->status_code/100 == 2) {
		...
		if (pubc->refresh_timer_cb != NULL)
		{
			pubc->timer.cb = pubc->refresh_timer_cb;
		}
		else
		{
