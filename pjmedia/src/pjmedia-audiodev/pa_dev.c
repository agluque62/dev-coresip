										...
										void *userData )
{
	...
	*/
	...
	nsamples = frameCount * stream->in_channel_count + stream->rec_buf_count;

	if (nsamples >= stream->in_samples_per_frame) 
	{
		...
		*/
		if (stream->rec_buf_count) {
			...
			chunk_count = stream->in_samples_per_frame - stream->rec_buf_count;
			pjmedia_copy_samples(stream->rec_buf + stream->rec_buf_count,
				(pj_int16_t*)input, chunk_count);

			frame.type = PJMEDIA_FRAME_TYPE_AUDIO;
			frame.buf = (void*) stream->rec_buf;
			frame.size = stream->in_samples_per_frame * stream->bytes_per_sample;
		...
		while (nsamples >= stream->in_samples_per_frame && status == 0) {
			...
			input = (pj_int16_t*) input + stream->in_samples_per_frame;
			nsamples -= stream->in_samples_per_frame;
			stream->rec_timestamp.u64 += stream->samples_per_frame / stream->channel_count;
		}
		...
		pjmedia_copy_samples(stream->rec_buf + stream->rec_buf_count,
			(pj_int16_t*)input, 
			frameCount * stream->in_channel_count);
		stream->rec_buf_count += frameCount * stream->in_channel_count;
	}
									 ...
									 void *userData )
{
	struct pa_aud_stream *stream = (struct pa_aud_stream*) userData;
	pj_status_t status = 0;
	unsigned nsamples_req = frameCount * stream->out_channel_count;
	...
	*/
	...
	while (nsamples_req && status == 0) {
		if (nsamples_req >= stream->out_samples_per_frame) {
			pjmedia_frame frame;

			frame.type = PJMEDIA_FRAME_TYPE_AUDIO;
			frame.buf = output;
			frame.size = stream->out_samples_per_frame *  stream->bytes_per_sample;
			...
			nsamples_req -= stream->out_samples_per_frame;
			output = (pj_int16_t*)output + stream->out_samples_per_frame;
		} else {
			pjmedia_frame frame;
			...
			pjmedia_copy_samples((pj_int16_t*)output, stream->play_buf, 
				nsamples_req);
			stream->play_buf_count = stream->out_samples_per_frame - nsamples_req;
			pjmedia_move_samples(stream->play_buf, 
				stream->play_buf+nsamples_req,
				stream->play_buf_count);
			nsamples_req = 0;
												 ...
												 pjmedia_aud_param *param)
{
	...
	param->in_channel_count = param->channel_count;
	param->out_channel_count = param->channel_count;
	param->samples_per_frame = adi.default_samples_per_sec * 20 / 1000;
	param->in_samples_per_frame = param->samples_per_frame;
	param->out_samples_per_frame = param->samples_per_frame;
												 ...
												 pjmedia_aud_stream **p_snd_strm)
{
	...
	rec_id = param->rec_id;
	if (rec_id < 0) {
		rec_id = pa_get_default_input_dev(param->in_channel_count);
		if (rec_id < 0) {
			/* No such device. */
	...
	stream->in_samples_per_frame = param->in_samples_per_frame;
	stream->bytes_per_sample = param->bits_per_sample / 8;
	stream->channel_count = param->channel_count;
	stream->in_channel_count = param->in_channel_count;
	stream->rec_cb = rec_cb;

	stream->rec_buf = (pj_int16_t*)pj_pool_alloc(pool, 
		stream->in_samples_per_frame * stream->bytes_per_sample);
	stream->rec_buf_count = 0;

	pj_bzero(&inputParam, sizeof(inputParam));
	inputParam.device = rec_id;
	inputParam.channelCount = param->in_channel_count;
	inputParam.hostApiSpecificStreamInfo = param->inHostApiSpecificStreamInfo;
	...
	paFrames = param->in_samples_per_frame / param->in_channel_count;
	...
	PJ_LOG(5,(THIS_FILE, "Opened device %s (%s) for recording, sample "
		...
		paRate, param->in_channel_count,
		param->bits_per_sample, param->in_samples_per_frame,
		paLatency));
												  ...
												  pjmedia_aud_stream **p_snd_strm)
{
	...
	if (play_id < 0) {
		play_id = pa_get_default_output_dev(param->out_channel_count);
		if (play_id < 0) {
			/* No such device. */
			return PJMEDIA_EAUD_NODEFDEV;
		}
	...
	stream->samples_per_sec = param->clock_rate;
	stream->samples_per_frame = param->samples_per_frame;
	stream->out_samples_per_frame = param->out_samples_per_frame;
	stream->bytes_per_sample = param->bits_per_sample / 8;
	stream->channel_count = param->channel_count;
	stream->out_channel_count = param->out_channel_count;
	stream->play_cb = play_cb;

	stream->play_buf = (pj_int16_t*)pj_pool_alloc(pool, 
		stream->out_samples_per_frame * 
		stream->bytes_per_sample);
	stream->play_buf_count = 0;

	pj_bzero(&outputParam, sizeof(outputParam));
	outputParam.device = play_id;
	outputParam.channelCount = param->out_channel_count;
	outputParam.hostApiSpecificStreamInfo = param->outHostApiSpecificStreamInfo;
	...
	/* Frames in PortAudio is number of samples in a single channel */
	paFrames = param->out_samples_per_frame / param->out_channel_count;
	...
	PJ_LOG(5,(THIS_FILE, "Opened device %d: %s(%s) for playing, sample rate=%d"
		...
		paRate, param->out_channel_count,
		param->bits_per_sample, param->out_samples_per_frame, 
		paLatency));
													...
													pjmedia_aud_stream **p_snd_strm)
{
	...
	stream->play_buf = (pj_int16_t*)pj_pool_alloc(pool, 
		stream->out_samples_per_frame * stream->bytes_per_sample);
	stream->play_buf_count = 0;
	...
	*/
	...
	PJ_LOG(5,(THIS_FILE, "Opened device %s(%s)/%s(%s) for recording and "
		"playback, sample rate=%d, Inch=%d, Outch=%d, "
		"bits=%d, %d in samples per frame, %d out samples per frame, input latency=%d ms, "
		"output latency=%d ms",
		paRecDevInfo->name, paRecHostApiInfo->name,
		paPlayDevInfo->name, paPlayHostApiInfo->name,
		paRate, param->in_channel_count, param->out_channel_count,
		param->bits_per_sample, param->in_samples_per_frame, param->out_samples_per_frame,
											 ...
											 pjmedia_aud_param *pi)
{
	...
	pi->in_channel_count = strm->in_channel_count;
	pi->out_channel_count = strm->out_channel_count;
	pi->samples_per_frame = strm->samples_per_frame;
	pi->in_samples_per_frame = strm->in_samples_per_frame;
	pi->out_samples_per_frame = strm->out_samples_per_frame;
