...
{
    int offset;
	pj_uint32_t ext_info = 0;
	pj_uint8_t *p_rtp_ext_info_aux = NULL;

    PJ_UNUSED_ARG(ses);
	pj_assert(ext_type_length != NULL);

	ext_type_length->length = 0;
	ext_type_length->profile_data = 0;
	*p_rtp_ext_info = NULL;
	*rtp_ext_length = 0;
    ...
    *hdr = (pjmedia_rtp_hdr*)pkt;
    ...
    if ((*hdr)->x) {
		...
		pj_memcpy(ext_type_length, ext, sizeof(pjmedia_rtp_ext_hdr));
		ext_info = *((pj_uint32_t*)(((pj_uint8_t*)pkt) + offset + sizeof(pj_uint32_t))); // | 0x02000000;
		p_rtp_ext_info_aux = (pj_uint8_t *) ext;
		p_rtp_ext_info_aux += sizeof(pj_uint32_t);
		*p_rtp_ext_info = (void *) p_rtp_ext_info_aux;		

		*rtp_ext_length = (pj_uint32_t) pj_ntohs(ext->length);
