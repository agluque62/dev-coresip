...
struct pjmedia_stream
{
	...
	pj_uint16_t muestra_anterior;
	pj_uint16_t muestra_paquete_anterior;
	pj_uint16_t muestra_paquete_anterior_mas_1;
	int num_muestras_iguales;
	int veces_incompleto;
	...
	pj_bool_t rx_only;			//Indica si es un recurso rx_only
...
*/
static unsigned long long GetTimeClimax(void)
{
...
static void send_keep_alive_packet(pjmedia_stream *stream, int ts_len)
{
	if (stream->ka_type == PJMEDIA_STREAM_KA_R2S)
	{
		...
		else
		{
			...
			if (0) //(PJMEDIA_RTP_RD_EX_GET_X(ext_val) == 1)
			{
				//Tenemos que enviar la extensi�n de cabecera sencilla m�s el RMM
				pj_memcpy((char*)stream->enc->out_pkt + pkt_len + sizeof(pj_uint32_t), &ext_val, sizeof(pj_uint32_t));

				ext_val = 0x43000000;		//Type RMM + length
				T1_u32 = (pj_uint32_t) T1;
				T1_u32 &= 0x7FFFFF;
				if (stream->NTP_synchronized) T1_u32 |= 0x800000;
				ext_val |= T1_u32;
				...
				pj_memcpy((char*)stream->enc->out_pkt + pkt_len + 2*sizeof(pj_uint32_t), buf, sizeof(pj_uint32_t));			
...
VOID CALLBACK TimerRoutine(PVOID lpParam, BOOLEAN TimerOrWaitFired)
{
   ...
   pjmedia_stream *stream = (pjmedia_stream *) pMyData->stream;

   PJ_UNUSED_ARG(TimerOrWaitFired);	   
   ...
   if (stream && stream->transport) {	//Comprobamos que siga la comunicación abierta
		...
		DeleteTimerQueueTimer(stream->hTimerQueue, pMyData->timer, NULL);
   }
   free(pMyData);
}
...
static pj_status_t put_frame_imp( pjmedia_port *port, const pjmedia_frame *frame )
{
	...
	pj_bool_t enviar_paquete_rtp = PJ_TRUE;
	 ...
	 if ((stream->rtp_ext_enabled) && req_MAM)
	 {
		rtp_hdr_size += sizeof(pj_uint32_t);
	 }
	...
	*/
	 ...
	 {
		//Si el recurso es rx_only. No se envía RTP independientemente del tipo de PTT
		if (stream->rx_only) {
			stream->enc_buf_pos = stream->enc_buf_count = 0;
			return PJ_SUCCESS;
		}
	       ...
	       (frame->type == PJMEDIA_FRAME_TYPE_EXTENDED))
    {
		...
		if (frame->size != 320)
		{
			PJ_LOG(3,(THIS_FILE,"INCIPALMA: put_frame_imp. Frame con size != 160"));
		}
		else
		{			
			pj_uint16_t *buffer_lineal = (pj_uint16_t *) frame->buf;
			for (i = 0; i < (frame->size / 2); i++)
			{
				if (*buffer_lineal != stream->muestra_anterior)
				{
					if (stream->num_muestras_iguales != 160 && stream->muestra_anterior > 5 && *buffer_lineal > 5)
					{
						PJ_LOG(3,(THIS_FILE,"INCIPALMA: put_frame_imp. Muestra %d se ha repetido %d veces y no 160. Muestra actual %d. Puede ser al principio o final de PTT", stream->muestra_anterior, stream->num_muestras_iguales, *buffer_lineal));
						if (stream->veces_incompleto++ > 3)
						{
							PJ_LOG(3,(THIS_FILE,"INCIPALMA: put_frame_imp. veces incompleto"));
							stream->veces_incompleto = 0;
						}
					}
					stream->num_muestras_iguales = 1;
					stream->muestra_paquete_anterior_mas_1 = *buffer_lineal;
				}
				else
				{
					stream->num_muestras_iguales += 1;
					if (stream->num_muestras_iguales == frame->size / 2)
					{
						if (*buffer_lineal == stream->muestra_paquete_anterior && *buffer_lineal > 5)
						{
							PJ_LOG(3,(THIS_FILE,"INCIPALMA: put_frame_imp. Dato/paquete %d repetido", *buffer_lineal));
						}
						else if (*buffer_lineal != stream->muestra_paquete_anterior_mas_1 && *buffer_lineal > 5)
						{
							PJ_LOG(3,(THIS_FILE,"INCIPALMA: put_frame_imp. Recibido %d cuando se esperaba %d", *buffer_lineal, stream->muestra_paquete_anterior_mas_1));
						}
						stream->muestra_paquete_anterior = *buffer_lineal;
						stream->muestra_paquete_anterior_mas_1 = stream->muestra_paquete_anterior + 1;
						if (stream->muestra_paquete_anterior_mas_1 == 0x100)
						{
							stream->muestra_paquete_anterior_mas_1 = 0;
						}
					}
				}

				stream->muestra_anterior = *buffer_lineal;
				buffer_lineal++;
			}
		}
	...
	buff_out = (pj_uint8_t *) frame_out.buf;
	buff_in = (pj_uint16_t *) frame->buf;
	for (i = 0; i < frame_out.size; i++)
	{
		*buff_out = (pj_uint8_t) *buff_in;
		buff_out++;
		buff_in++;
	}
	...
	return PJ_SUCCESS;
	 ...
	 { 
		 ...
		 pj_uint32_t ex_val = stream->rtp_ext_tx_info;
		 ...
		 {			
			...
			PJMEDIA_RTP_RD_EX_SET_X(ex_val, 1);
			 
			//Se env�a mensaje RMM. Type 4, length 3, 1 bit TQV, 23 bits T1 			
			PJMEDIA_RTP_RD_EX_SET_TYPE(ex_val, 0x4);
			PJMEDIA_RTP_RD_EX_SET_LENGTH(ex_val, 0x3);

			T1 = GetTimeClimax();			

			//TRC_((THIS_FILE, "TRAZA: Envio T1 cona audio %lld (0x%llX)", T1, T1));
			...
			PJMEDIA_RTP_RD_EX_SET_VALUE(ex_val, value);			

			pj_memcpy((char*)channel->out_pkt + sizeof(pjmedia_rtp_hdr) + sizeof(pj_uint32_t), &ex_val, sizeof(pj_uint32_t));

			ex_val = (pj_uint32_t) T1;
			ex_val &= (pj_uint32_t) 0xFFFF;
			...
			for (i = 0; i < 4; i++)
			{
				buf[i] = (pj_uint8_t) (ex_val >> (24-i*8));
			}

			pj_memcpy((char*)channel->out_pkt + sizeof(pjmedia_rtp_hdr) + 2*sizeof(pj_uint32_t), buf, sizeof(pj_uint32_t));
	...
	if (stream->Perdidos && (!(pj_rand()%stream->Perdidos)))			//Perdemos aletoriamente paquetes
	{
		enviar_paquete_rtp = PJ_FALSE;
	}
	
	if (stream->Duplicados && (!(pj_rand()%stream->Duplicados)))		//Duplicamos aletoriamente paquetes
	{
		if (stream->LatMax)
		{
			int latency = stream->LatMax == stream->LatMin? stream->LatMin : stream->LatMin + (pj_rand() % (stream->LatMax - stream->LatMin));
			...
			CreateTimerQueueTimer( &MyData->timer, stream->hTimerQueue, TimerRoutine, MyData , latency, 0, WT_EXECUTEONLYONCE);			
		}
		else
		{
	...
	/* Hemos recibido un paquete v�lido */
	 if (stream->rtp_ext_enabled)
	 {
		 ...
		 if (stream->reenv_info_count != 0)
		 {
			...
			{
				pj_app_cbs.on_stream_rtp_ext_info_changed(stream, (void *) &ext_type_length, rtp_ext_info, p_rtp_ext_info, rtp_ext_length);
			}
		 }		 
	 }
...

{
	...
	*p_stream = NULL;
	...
	stream->muestra_paquete_anterior_mas_1 = 1;
    ...
    *p++ = '.'; *p++ = 'o'; *p++ = 'r'; *p++ = 'g';
	...
	stream->hTimerQueue = CreateTimerQueue();
...
PJ_DEF(void) pjmedia_stream_force_set_impairments(pjmedia_stream * stream, int Perdidos, int Duplicados, int LatMin, int LatMax)
{
	pj_assert(stream != NULL);

	stream->Perdidos = Perdidos;
	stream->Duplicados = Duplicados;
	stream->LatMin = LatMin;
	stream->LatMax = LatMax;
}


PJ_DEF(void) pjmedia_stream_set_rx_only(pjmedia_stream * stream, pj_bool_t rx_only)
{
	if (stream == NULL) return;
	stream->rx_only = rx_only;
}
...
PJ_DEF(pj_status_t) pjmedia_stream_destroy( pjmedia_stream *stream )
{
	...
	DeleteTimerQueueEx(stream->hTimerQueue, INVALID_HANDLE_VALUE);  //Se eliminan todos los timers, pero esperamos a que finalizen.
