...
{    
    ...
    */
    ...
    for ( fmti = 0; fmti < local_m->desc.fmt_count; ++fmti ) 
	{
		...
		/** 20160530 AGL. Ignorar el codec 123 (R2S) */
		if (pt==123)
			continue;
	...
	if (attr) 
	{
			...
			(pj_strnicmp2(&attr->value, "Radio-Idle",10) == 0) )
		{
			...
			//attr = pjmedia_sdp_attr_find2(local_m->attr_count, local_m->attr, "sigtime", NULL);
			//si->sigtime = attr ? pj_strtoul(&attr->value) : PJMEDIA_DEFAULT_SIGTIME;
			//attr = pjmedia_sdp_attr_find2(local_m->attr_count, local_m->attr, "ptt_rep", NULL);
			//si->ptt_rep = attr ? pj_strtoul(&attr->value) : PJMEDIA_DEFAULT_PTT_REP;
			...
			if (!attr_pttid)
			{
				//Si el type del local es de tipo radio y ptt-id del remote no esta presente entonces somos una radio
				si->radio_ua = PJ_TRUE;
