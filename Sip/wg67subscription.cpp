...
void WG67Subscription::wg67_on_state(pjsip_evsub *sub, pjsip_event *event)
{
	...
	if (wg67) 
	{
		PJ_LOG(4,(THIS_FILE, "WG67 KEY-IN subscription to %.*s is %s",
...
: _Pool(NULL), _Module(NULL)
{
	...
	try
	{
		...
		else 
		{
			...
			PJ_CHECK_STATUS(st, ("Unable to generate Contact header for WG67 KEY-IN"));
		}

		st = pjsip_dlg_create_uac(pjsip_ua_instance(), &acc->cfg.id, &contact, &_Dst, NULL, &dlg);
		PJ_CHECK_STATUS(st, ("Unable to create dialog for WG67 KEY-IN"));

		st = pjsip_wg67_create_uac(dlg, &wg67_callback, PJSIP_EVSUB_NO_EVENT_ID, &_Module);
		PJ_CHECK_STATUS(st, ("Unable to create WG67 KEY-IN client"));
		...
		PJ_CHECK_STATUS(st, ("Unable to create initial WG67 KEY-IN SUBSCRIBE"));

		pjsua_process_msg_data(tdata, NULL);

		st = pjsip_wg67_send_request(_Module, tdata);
		PJ_CHECK_STATUS(st, ("Unable to send initial WG67 KEY-IN SUBSCRIBE"));
	}
	catch (...)
	{
		if (_Module)
