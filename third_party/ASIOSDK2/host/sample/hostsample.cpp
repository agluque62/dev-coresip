...
{	// the actual processing callback.
	...
	if (lasttm != 0)
	{
		unsigned diff = asioDriverInfo.sysRefTime - lasttm;
	...
	lasttm = asioDriverInfo.sysRefTime;
			...
			if (init_asio_static_data (&asioDriverInfo) == 0)
			{
				...
				if (create_asio_buffers (&asioDriverInfo) == ASE_OK)
				{
					if (ASIOStart() == ASE_OK)
					{
						...
						fprintf(stdout, "\n\nMaxDiff=%d / MinDiff=%d / AvgDiff=%d", maxDiff, minDiff, sumDiff / numCalls);
					}
					ASIODisposeBuffers();
				}
			}
